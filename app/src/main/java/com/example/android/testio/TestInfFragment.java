...
//Фрагмент информации о тесте
public class TestInfFragment extends Fragment {

    private RecyclerView mRecyclerView2;
    FirebaseUser currentU = FirebaseAuth.getInstance().getCurrentUser();
    String uId = currentU.getUid();
    public TestInfFragment() {

    }
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.fragment_passed_test_inf, null);

        mRecyclerView2 = (RecyclerView) v.findViewById(R.id.lul2);
        if(uId!=null){
            new FirebaseDatabaseHelper().readQuestions(new FirebaseDatabaseHelper.DataStatus() {
                ...
                public void DataIsLoaded4(List<QuestionsObject> Questions, List<String> keys) {
                    new RecyclerView_Config().setConfig4(mRecyclerView2,getActivity(), Questions,keys);
                }
